...
def compute_bcubed(all_logits, all_labels, **kwargs):
    eval_results = []
    all_pred_clusters = []
    for i in range(len(all_logits)):
        ...
        # print(prob.size())
        # loss = loss + torch.sum(torch.log(torch.sum(torch.mul(prob, filled_labels), dim=1))) * -1
        ...
        all_pred_clusters.append(pred_clusters)
    # b cubed
    res = evaluate_documents(eval_results, b_cubed)
    return {"micro_f1": res[-1]}, all_pred_clusters


def get_pred_clusters(all_logits, **kwargs):
    all_pred_clusters = []
    for i in range(len(all_logits)):
        prob = all_logits[i]
        pred_clusters, pred_event2cluster = get_predicted_clusters(prob)
        all_pred_clusters.append(pred_clusters)
    return all_pred_clusters